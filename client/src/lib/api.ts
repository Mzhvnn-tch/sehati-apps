...
async function apiCall<T>(
  ...
  if (!response.ok) {
    try {
      ...
      if (e.status) throw e; // Re-throw if it already has status
      const errorObj = new Error(e.message || `API request failed with status ${response.status}`);
      (errorObj as any).status = response.status;
      throw errorObj;
    }
  }
...
export async function getPendingDoctors(): Promise<{ doctors: User[] }> {
  return apiCall("/admin/doctors/pending");
}

export async function approveDoctor(userId: string, txHash: string): Promise<{ success: boolean }> {
  return apiCall("/admin/approve-doctor", {
    method: "POST",
